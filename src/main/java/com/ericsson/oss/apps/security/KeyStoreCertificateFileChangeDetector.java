...
public class KeyStoreCertificateFileChangeDetector {
    ...
    public void checkForUpdates() {
        ...
        final Path dirPath = Paths.get(tlsConfig.getKeystoreCertFilePath());
        Path certPath = null;
        Path keyPath = null;
        ...
        try (final DirectoryStream<Path> dirStream = Files.newDirectoryStream(dirPath)) {
            ...
            log.debug("Completed check for keystore certificate and key updates");
    ...
    private Collection<Certificate> loadCert(final Path path, final FileTime fileLastModifiedTime)
            throws CertificateHandlingException, IOException {
    ...
    private KeyStore readKeyStore() {
        ...
        try {
            keystore = CustomX509ManagerUtils.loadStore(keyStoreConfig.getAppStorePath(), keyStoreConfig.getAppKeyStorePass(),
