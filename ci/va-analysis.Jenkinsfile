...
def bob = "./bob/bob"
def ruleset = "ci/va-analysis-ruleset.yaml"
...
if (env.RELEASE) {
    stage('Additional VA scans') {
        parallel(
            ...
            "Hadolint-UI": {
                script {
                    sh "${bob} -r ${ruleset} hadolint-scan-ui"
                    echo "Evaluating Hadolint Scan Resultcodes..."
                    sh "${bob} -r ${ruleset} evaluate-design-rule-check-resultcodes"
                    archiveArtifacts "build/va-reports/hadolint-scan-ui/**.*"
                }
            },
            "Trivy": {
                script {
                    ...
                    sh "${bob} -r ${ruleset} trivy-inline-scan-ui"
                    archiveArtifacts "build/va-reports/trivy-reports-ui/**.*"
                    archiveArtifacts "trivy_metadata.properties"
                }
            },
            "X-Ray": {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'XRAY_SELI_ARTIFACTORY', usernameVariable: 'XRAY_USER', passwordVariable: 'XRAY_APIKEY')]) {
                        ci_pipeline_scripts.retryMechanism("${bob} -r ${ruleset} fetch-xray-report-ui", 3)
                    }
                    archiveArtifacts "build/va-reports/xray-reports-ui/xray_report.json"
                    archiveArtifacts "build/va-reports/xray-reports-ui/raw_xray_report.json"
                    sleep(60)
                    withCredentials([usernamePassword(credentialsId: 'XRAY_SELI_ARTIFACTORY', usernameVariable: 'XRAY_USER', passwordVariable: 'XRAY_APIKEY')]) {
            ...
            "Anchore-Grype": {
                script {
                    sh "${bob} -r ${ruleset} anchore-grype-scan-ui"
                    archiveArtifacts "build/va-reports/anchore-reports-ui/**.*"
    ...
    stage('Generate Vulnerability report V2.0') {
        script {
            sh "${bob} -r ${ruleset} delete-images:delete-internal-ui-image"
            sh "${bob} -r ${ruleset} generate-UI-VA-report-V2:no-upload"
            archiveArtifacts allowEmptyArchive: true, artifacts: 'build/va-reports/UI-Vulnerability_Report_2.0.md'
