...
checkServiceYAML(){
    echo "SUCCESS: service.yaml does not contain prometheus annotation as the metrics are already exposed via the pod";
}

checkDeploymentYAML(){
    SERVICE_NAME=$1
    if grep -q "{{- include \"$SERVICE_NAME.annotations\" . | nindent [0-9] }}" ./charts/$SERVICE_NAME/templates/deployment.yaml; then
        ...
        echo -e "What is needed:"
        echo -e "{{- include \"$SERVICE_NAME.annotations\" . | nindent [0-9] }}"
...
#     Commented out as we have created our own custom restTemplate bean in the RestTemplateConfiguration.java file.
#     This custom bean still exposes the Prometheus endpoints needed to gather metrics
