...
RUN echo "${USER_ID}:x:${USER_ID}:${USER_ID}:An Identity for ${USER_NAME}:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD target/${JAR_FILE} eric-oss-auto-config-consistency-app.jar
...
#     -jar eric-oss-auto-config-consistency-app.jar"]
...
LABEL \
    org.opencontainers.image.title=eric-oss-auto-config-consistency \
