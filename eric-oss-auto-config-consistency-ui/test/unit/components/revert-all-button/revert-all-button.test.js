...
import RevertAllButton from '../../../../src/components/revert-all-button/revert-all-button.js';

/* eslint-disable import/no-named-default */
import { default as LOCALE_EN_US } from '../../../../src/components/revert-all-button/locale/en-us.json';
/* eslint-disable import/no-named-default */
 ...
 */
describe('Given: An <e-revert-all-button>', () => {
  before(() => {
    RevertAllButton.register();
  ...
  describe(`When: A REST call is made to ${AUDIT_RESULTS_API} with any 'In progress' changes`, () => {
    it(`Then: The <e-revert-all-button> is disabled`, async () => {
      fetchStub
        .withArgs(sinon.match(AUDIT_RESULTS_API))
        .resolves(MOCK_POPULATED_AUDITS_RESPONSE_CONTAINS_IN_PROGRESS);

      const component = await fixture(
        html`<e-revert-all-button execution-id=${EXECUTION_ID}
          >${BUTTON_NAME}</e-revert-all-button
        >`,
      );
